...
def addingNewMachine(ws, seal, authId, host, machine):
    ...
    waitForResponse(
        ws,
        '[48,6152715661537792,{"receive_progress":false},"api.konfigd.get",["08593f34-0d7c-39fd-7eae-cef72dd3cabc","exec konfne params --json '
...
def addingAllSignalForMachine(ws, machineName, host, seal, authId):
    ...
    getCheckingSignalsOnMachine = waitGetResponse(
        f"http://{host}/rest/tree/node?username={authId}&seal={seal}&with_alerts=1&filter_tags=0&path=/Tree/{machineName}"
    )
    message = waitGetResponse(
        f"http://{host}/rest/tree/node?username={authId}&seal={seal}&with_alerts=1&filter_tags=0&path=/Tree/{machineName}"
    )
