...
@allure.step('Searching mp3 files in lanlot-data')
def soundSearching(seal):
    host = getFromConfig("host")
    username = getFromConfig("login")
    getMusicList = requests.get(f'http://{host}/rest/account/ls?dir=var/lanlot-data/_sounds&ext=mp3&seal={seal}&username={username}')
    message = getMusicList.text
    assert 'data' in message and 'var/lanlot-data/' in message, "Cannot find sounds"
