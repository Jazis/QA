...
def waitForResponse(ws, commandLine, _waitResponse = True, timeout=10, _retryTimes = 2):
    ...
    for i in range(len(commandLine.split(','))):
        if commandLine.split(',')[i].isdigit() and len(commandLine.split(',')[i]) >= 14 and commandLine.split(',')[i-1].split('[')[1].isdigit() and len(commandLine.split(',')[i-1].split('[')[1]) < 3:
            commandLineNumber = commandLine.split(',')[i]
    if commandLineId == None: commandLineId = commandLineNumber
    if commandLineNumber == None: commandLineNumber = commandLineId
    while counter0 < _retryTimes:
        ...
        if _waitResponse == False:
            return msg
        msg = ""
        while counter1 <= timeout and commandLineId not in msg or commandLineNumber not in msg:
            try:
                msg = ws.recv()
                if msg != None or msg != '' and commandLineId in msg or commandLineNumber in msg:
                    return msg
                else:
                    time.sleep(1)
                    counter1 +=1
                ...
                counter1 +=1
    assert commandLineId in msg or commandLineNumber in msg, "Something went wrong with WebSocket response"
    return msg