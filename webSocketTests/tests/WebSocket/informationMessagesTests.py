...
def addNewMessage(ws, authId, seal, host):
    ...
    checkList(ws, "name")
    waitForResponse(ws, r'[48,2750552443725732,{"receive_progress":false},"api.info_msg.create_template",[],{"template":{"name":"' + temp.testMessageName + '","content":"<span>{template}</span>"}}]')
    waitForResponse(ws, '[48,89566425095436,{"receive_progress":false},"api.info_msg.get_template",[],{"name":"' + temp.testMessageName + '"}]')
...
def removeMessage(ws):
    ...
    waitForResponse(ws, r'[48,1621413467715036,{"receive_progress":false},"api.info_msg.delete_template",[],{"name":"' + temp.testMessageName + r'"}]')
    message = waitForResponse(ws, r'[48,8407576860288522,{"receive_progress":false},"api.info_msg.get_templates",[]]')
    assert temp.testMessageName not in message, "The message was not deleted"

def removeParam(ws):
    logging.info("Removing parameter...")
    waitForResponse(ws, r'[48,694249696132376,{"receive_progress":false},"api.info_msg.save_template",[],{"template":{"params":{"' + temp.testMessageParameter + r'":null},"name":"TEST_NAME_OF_MESSAGE"}}]')
   
def addParam(ws):
    logging.info("Adding parameter " + temp.testMessageParameter + " in " + temp.testMessageName)
    message = waitForResponse(ws, r'[48,784178240428412,{"receive_progress":false},"api.info_msg.save_template",[],{"template":{"params":{"' + temp.testMessageParameter + r'":""},"name":"' + temp.testMessageName + r'"}}]')
    assert r'[{"data":{"success":true}}]]' in message, "Cannot add new parameter"
    
def checkList(ws, data = ""):
    logging.info("Checking list for " + data + "...")
    message = waitForResponse(ws, '[48,980082037499158,{"receive_progress":false},"api.info_msg.get_templates",[]]')
...
def setDescription(ws):
    ...
    message = waitForResponse(ws, r'[48,7607675748793548,{"receive_progress":false},"api.info_msg.save_template",[],{"template":{"params":{},"description":"test description","name":"' + temp.testMessageName + r'"}}]')
