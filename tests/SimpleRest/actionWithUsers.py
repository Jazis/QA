...
def disableNewAcccount(seal, host, username):
    ...
    requestForDeleteNewAcc = requests.delete(f'http://{host}/rest/access/userdel?users=%5B%22{temp.userId}%22%5D&force=0&username={username}&seal={seal}').text
...
def deleteNewAccount(seal, host, username):
    ...
    deleteNewAccountRequest = requests.delete(f'http://{host}/rest/access/userdel?users=%5B%22{temp.userId}%22%5D&force=1&username={username}&seal={seal}').text
    assert f'"data":["{temp.userId}","user=uid={temp.userId},ou=Users,dc=lancelot"]' in deleteNewAccountRequest, "Something went wrong with delete new account"
