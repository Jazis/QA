...
@allure.title("Transaction auth pending refund")
def test_auth_pending_refund(api_request_context: APIRequestContext) -> None:
    token = api_post_auth_trans(api_request_context, 50, VISA, '{"approved":true}')
    data = [
        {
            "id": token,
            "data": {
