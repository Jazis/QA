...
@allure.title("Transaction settled pending")
def test_auth_settled_pending(api_request_context: APIRequestContext) -> None:
    token = api_post_auth_trans(api_request_context, 70, VISA, '{"approved":true}')
    data = [
        {
            "id": str(uuid.uuid4()),
            "data": {
