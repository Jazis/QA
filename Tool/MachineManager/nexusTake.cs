    ...
    public partial class nexusTake : Form
    {
		...
		public void downloadingNeededModules()
		{
			foreach (object item in this.checkedListBox1.CheckedItems)
			{
				...
				HttpWebRequest request = (HttpWebRequest)WebRequest.Create("https://nexus.ad.pkcc-ps.ru/service/extdirect");
				request.Method = "POST";
				...
				string urlLink = string.Format("https://nexus.ad.pkcc-ps.ru/repository/{0}/{1}", item, lastVersionOfmaster);
		...
		public TreeNodeCollection getDatatFromStorage(TreeView treeNode, int nexusPathIndex)
        {
			...
			HttpWebRequest request0 = (HttpWebRequest)WebRequest.Create("https://nexus.ad.pkcc-ps.ru/service/extdirect");
